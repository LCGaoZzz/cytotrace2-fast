					...
					match par {
						...
						Par::Rayon(nthreads) => {
...

							{
								...
								// fork: still exactly `nthreads` slabs with identical
								// per-slab arithmetic (bit-identical partials), but the
								// slabs are executed by a small fixed number of rayon
								// tasks with interleaved slab assignment. wide fan-out
								// saturates DRAM bandwidth and the hybrid P/E topology
								// inverts the scaling past ~6 concurrent streams.
								static TRI_GROUPS: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
								let ngroups = (*TRI_GROUPS.get_or_init(|| {
									std::env::var("C2RUST_TRI_GROUPS")
										.ok()
								...
								.max(1);
								let zlen = z2.rb().nrows();
								let z2cs = z2.rb().col_stride() as usize;
								let z2data = crate::linalg::evd::tridiag_evd_shared::SharedMut(z2.rb_mut().as_ptr_mut());
								let a22s = crate::linalg::evd::tridiag_evd_shared::SyncRef(A22);
								let y2s = crate::linalg::evd::tridiag_evd_shared::SyncRef(y2);
								let w2s = crate::linalg::evd::tridiag_evd_shared::SyncRef(w2.rb());
								let u2s = crate::linalg::evd::tridiag_evd_shared::SyncRef(u2.rb());
								let x2s = crate::linalg::evd::tridiag_evd_shared::SyncRef(x2.rb());
								let k1v = k1;
								let nv = n;
								(0..ngroups).into_par_iter().for_each(|gid| {
									let mut idx = gid;
									while idx < nthreads {
										...
										let mut A =
											unsafe { a22s.get().subcols(first, ncols).subrows(first, nrows).const_cast() };

										{
											let zfull: &mut [T] = unsafe {
												core::slice::from_raw_parts_mut(z2data.idx(idx * z2cs), zlen)
											};
											let mut z2c = ColMut::from_slice_mut(zfull);
											let y2 = unsafe { y2s.get().subrows(first, ncols).const_cast() };
											let mut z2s = z2c.rb_mut().subrows_mut(first, nrows);

											let ry2 = w2s.get().subrows(first, ncols);
											let rz2 = w2s.get().subrows(first, nrows);

											let u0 = u2s.get().subrows(first, nrows);
											let u1 = u2s.get().subrows(first, ncols);
											let u2 = x2s.get().subrows(first, ncols);
											let v2 = x2s.get().subrows(first, nrows);
											...
											tridiag_fused_op(
												...
												v2,
												copy(f),
												nv.next_power_of_two() - (k1v + 1) - first,
											);
											z2c.rb_mut().subrows_mut(0, first).fill(zero());
										}
										idx += ngroups;
									}
								});
							...
							// fork: reduce the per-slab z2 partials into y2 with per-row
							// idx-ordered accumulation, parallel over rows. each row's
							// addition chain (y + z2[0] + z2[1] + ...) is unchanged, so
							// the result is bit-identical to the serial column loop.
							{
								let nrows = z2.rb().nrows();
								let zcs = z2.rb().col_stride() as usize;
								let zdata = crate::linalg::evd::tridiag_evd_shared::SharedConst(z2.rb().as_ptr());
								let ydata = crate::linalg::evd::tridiag_evd_shared::SharedMut(y2.rb_mut().as_ptr_mut());
								(0..nrows)
									.into_par_iter()
									.with_min_len(2048)
									.for_each(|i| {
										let mut acc = unsafe { (*ydata.idx(i)).clone() };
										for idx in 0..nthreads {
											acc = acc + unsafe { (*zdata.idx(idx * zcs + i)).clone() };
										}
										unsafe {
											*ydata.idx(i) = acc;
										}
