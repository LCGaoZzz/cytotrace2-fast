...
/// fork (rust-cli-evd-parallel): tiny Send/Sync raw-pointer wrappers shared by
/// the parallelized EVD/tridiag kernels. closures must capture the whole
/// wrapper (never its field) to stay Send + Sync.
#[doc(hidden)]
pub mod tridiag_evd_shared {
	pub struct SharedMut<T>(pub *mut T);
	unsafe impl<T> Send for SharedMut<T> {}
	unsafe impl<T> Sync for SharedMut<T> {}
	impl<T> SharedMut<T> {
		#[inline(always)]
		pub unsafe fn idx(&self, i: usize) -> *mut T {
			unsafe { self.0.add(i) }
		}
	}
	pub struct SharedConst<T>(pub *const T);
	unsafe impl<T> Send for SharedConst<T> {}
	unsafe impl<T> Sync for SharedConst<T> {}
	impl<T> SharedConst<T> {
		#[inline(always)]
		pub unsafe fn idx(&self, i: usize) -> *const T {
			unsafe { self.0.add(i) }
		}
	}
	pub struct SyncRef<X>(pub X);
