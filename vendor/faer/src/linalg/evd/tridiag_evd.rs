...
// fork (rust-cli-evd-parallel): rayon parallelism for the serial O(n^2) merge
// phases of the divide & conquer tridiagonal EVD. every parallelized loop has
// disjoint writes and unchanged per-element arithmetic order, so results stay
// bit-identical to the upstream serial implementation.
#[cfg(feature = "rayon")]
use rayon::iter::{IntoParallelIterator, ParallelIterator};

/// wrapper to share read-only faer view types across rayon closures
/// (view types hold raw pointers and are not automatically Sync).
/// closures must use `get()` so the whole wrapper is captured, never its field.
struct SyncRef<X>(X);
unsafe impl<X> Send for SyncRef<X> {}
unsafe impl<X> Sync for SyncRef<X> {}
impl<X: Copy> SyncRef<X> {
	#[inline(always)]
	fn get(&self) -> X {
		self.0
	}
...
/// raw mutable pointer shareable across rayon closures (disjoint indexed writes)
struct SharedMut<T>(*mut T);
unsafe impl<T> Send for SharedMut<T> {}
unsafe impl<T> Sync for SharedMut<T> {}
impl<T> SharedMut<T> {
	#[inline(always)]
	unsafe fn idx(&self, i: usize) -> *mut T {
		unsafe { self.0.add(i) }
	}
}

/// raw const pointer shareable across rayon closures
struct SharedConst<T>(*const T);
unsafe impl<T> Send for SharedConst<T> {}
unsafe impl<T> Sync for SharedConst<T> {}
impl<T> SharedConst<T> {
	#[inline(always)]
	unsafe fn idx(&self, i: usize) -> *const T {
		unsafe { self.0.add(i) }
	}
}
...
fn compute_eigenvalues<T: RealField>(
	...
	// fork: each secular-equation root depends only on the shared read-only
	// (d, z, rho) inputs, so the per-i root search parallelizes with identical
	// per-root arithmetic.
	let d_sub = SyncRef(d.subrows(0, n));
	let z_sub = SyncRef(z.subrows(0, n));
	let z_full = SyncRef(z);
	let mus_ptr = SharedMut(mus.rb_mut().as_ptr_mut());
	let shifts_ptr = SharedMut(shifts.rb_mut().as_ptr_mut());
	#[cfg(feature = "rayon")]
	{
		(0..n).into_par_iter().for_each(|i| {
			let (shift, mu) =
				compute_eigenvalue_root(d_sub.get(), z_sub.get(), z_full.get(), rho.clone(), rho_recip.clone(), i, n);
			unsafe {
				*shifts_ptr.idx(i) = shift;
				*mus_ptr.idx(i) = mu;
			}
		});
	}
...
/// fork: body of one secular-equation root search, extracted so it can run on a
/// rayon worker. `d`/`z` are the subviews over the non-deflated leading block,
/// `z_full` is only used for the trailing eigenvalue's right bracket bound,
/// exactly like the original in-loop shadowing.
#[math]
fn compute_eigenvalue_root<T: RealField>(
	d: ColRef<'_, T, usize, ContiguousFwd>,
	z: ColRef<'_, T, usize, ContiguousFwd>,
	z_full: ColRef<'_, T, usize, ContiguousFwd>,
	rho: T,
	rho_recip: T,
	i: usize,
	n: usize,
	...
	let right = if last { d[i] + rho * z_full.squared_norm_l2() } else { copy(d[i + 1]) };
...
/// fork: per-i new_zi product chain (see "perturb z" comment upstream).
#[math]
fn compute_new_zi<T: RealField>(
	permuted_diag: ColRef<'_, T, usize, ContiguousFwd>,
	mus: ColRef<'_, T, usize, ContiguousFwd>,
	shifts: ColRef<'_, T, usize, ContiguousFwd>,
	old_zi: T,
	i: usize,
	non_deflated: usize,
) -> T {
...
/// fork: fill + normalize one eigenvector column (row-major slice of a
/// column-major repaired_u). Mirrors the original per-column code exactly.
#[math]
fn fill_eigenvector_col<T: RealField>(
	col: &mut [T],
	pj: usize,
	pl_after: &[usize],
	permuted_z: ColRef<'_, T, usize, ContiguousFwd>,
	permuted_diag: ColRef<'_, T, usize, ContiguousFwd>,
	mus: ColRef<'_, T, usize, ContiguousFwd>,
	shifts: ColRef<'_, T, usize, ContiguousFwd>,
	non_deflated: usize,
	...
	if pj >= non_deflated {
		...
		for (i, &pl_after) in pl_after[..non_deflated].iter().enumerate() {
			...
			col[pl_after] = zi / ((di - shift_j) - mu_j);
		}
		for &pl_after in &pl_after[non_deflated..non_deflated + deflated] {
			col[pl_after] = zero();
		}

		let inv_norm = recip(ColRef::from_slice(&*col).norm_l2());
		for x in col.iter_mut() {
			*x = *x * inv_norm;
		}
	}
...
/// fork: householder repair of one eigenvector column segment. The dot product
/// views are constructed with the same (dyn-stride) types as the original code
/// so the reduction order is unchanged.
#[math]
fn householder_repair_col<T: RealField>(col: &mut [T], householder: &[T], tau_inv: T) {
	let lhs = ColRef::from_slice(householder).as_dyn_stride().transpose();
	let rhs = ColRef::from_slice(&*col).as_dyn_stride();
	let dot = tau_inv * dot::inner_prod(lhs, Conj::No, rhs, Conj::No);
	for (u, h) in col.iter_mut().zip(householder.iter()) {
		*u = *u - dot * *h;
	}
}
...
fn divide_and_conquer_recurse<T: RealField>(
	...
	// fork: per-i product chains are independent (reads only diag/mus/shifts,
	// writes only z[i]) -> parallel over i, identical arithmetic per element.
	{
		let pd_ro = SyncRef(permuted_diag.rb());
		let mus_ro = SyncRef(mus.rb());
		let shifts_ro = SyncRef(shifts.rb());
		let pz_ptr = SharedMut(permuted_z.rb_mut().as_ptr_mut());
		#[cfg(feature = "rayon")]
		{
			(0..non_deflated).into_par_iter().for_each(|i| {
				let old_zi = unsafe { (*pz_ptr.idx(i)).clone() };
				let new_zi = compute_new_zi(pd_ro.get(), mus_ro.get(), shifts_ro.get(), old_zi, i, non_deflated);
				unsafe {
					*pz_ptr.idx(i) = new_zi;
				}
			});
		...
		for i in 0..non_deflated {
			let (permuted_diag, mus, shifts) = (pd_ro.get(), mus_ro.get(), shifts_ro.get());
			let old_zi = copy(permuted_z[i]);
			let new_zi = compute_new_zi(permuted_diag, mus, shifts, old_zi, i, non_deflated);
			(permuted_z[i] = new_zi);
		}
	}
	...
	// fork: each eigenvector lives in its own column of repaired_u -> parallel
	// over columns with identical per-column arithmetic (same fill order, same
	// norm reduction over the same contiguous column).
	{
		let cs = repaired_u.rb().col_stride() as usize;
		let ru_ptr = SharedMut(repaired_u.rb_mut().as_ptr_mut());
		let pr_ro: &[usize] = pr;
		let pl_after_ro: &[usize] = pl_after;
		let pz_ro = SyncRef(permuted_z.rb());
		let pd_ro = SyncRef(permuted_diag.rb());
		let mus_ro = SyncRef(mus.rb());
		let shifts_ro = SyncRef(shifts.rb());
		#[cfg(feature = "rayon")]
		{
			(0..n).into_par_iter().for_each(|j| {
				let col: &mut [T] = unsafe { core::slice::from_raw_parts_mut(ru_ptr.idx(j * cs), n) };
				fill_eigenvector_col(
					col,
					pr_ro[j],
					pl_after_ro,
					pz_ro.get(),
		...
		#[cfg(not(feature = "rayon"))]
		for (j, &pj) in pr_ro.iter().enumerate() {
			let col: &mut [T] = unsafe { core::slice::from_raw_parts_mut(ru_ptr.idx(j * cs), n) };
			fill_eigenvector_col(
				col,
				pj,
	...
	if applied_householder {
		...
		while idx < n {
			...
			if run_len > 1 {
				...
				let cs = repaired_u.rb().col_stride() as usize;
				let ru_ptr = SharedMut(repaired_u.rb_mut().as_ptr_mut());
				let hh_ptr = SharedConst(householder.as_ptr());
				let tau_inv = recip(tau);

				// fork: the householder update of column j only touches column j
				// -> parallel over columns, identical dot product inputs/order.
				#[cfg(feature = "rayon")]
				{
					(0..n).into_par_iter().for_each(|j| {
						let col: &mut [T] =
							unsafe { core::slice::from_raw_parts_mut(ru_ptr.idx(j * cs + idx), run_len) };
						let hh: &[T] = unsafe { core::slice::from_raw_parts(hh_ptr.idx(0), run_len) };
						householder_repair_col(col, hh, tau_inv.clone());
					});
				}
				#[cfg(not(feature = "rayon"))]
				for j in 0..n {
		...
		// fork: column j of the permutation gather only touches column j
		{
			let cs_ru = repaired_u.rb().col_stride() as usize;
			let cs_tmp = tmp.rb().col_stride() as usize;
			let ru_ro = SharedConst(repaired_u.rb().as_ptr());
			let tmp_w = SharedMut(tmp.rb_mut().as_ptr_mut());
			let pb: &[usize] = pl_before;
			#[cfg(feature = "rayon")]
			{
				(0..n).into_par_iter().for_each(|j| {
					let src: &[T] = unsafe { core::slice::from_raw_parts(ru_ro.idx(j * cs_ru), n) };
					let dst: &mut [T] = unsafe { core::slice::from_raw_parts_mut(tmp_w.idx(j * cs_tmp), n) };
					for (i, &plb) in pb.iter().enumerate() {
						dst[plb] = src[i].clone();
					}
				});
			}
			#[cfg(not(feature = "rayon"))]
			for j in 0..n {
				for (i, &pl_before) in pb.iter().enumerate() {
